    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    setSlider(microTuneSlider, -1.0f, 1.0f, 0.0f);

    setLabel(keyFreqLabel);
    setLabel(cleaningNoiseLabel);
    setLabel(microTuneLabel);
    ...
    microTuneSliderAttch = std::make_unique<Attachment>(audioProcessor.apvts, "MicroTune", microTuneSlider);
...
void LaplandAudioProcessorEditor::resized()
{
    ...
    cleaningNoiseSlider.setBounds(100, Y, SliderSide, SliderSide);
    cleaningNoiseLabel.setBounds(cleaningNoiseSlider.getX(), cleaningNoiseSlider.getY() - 15, SliderSide, 20);

    microTuneSlider.setBounds(cleaningNoiseSlider.getRight(), Y, sliderWidth, sliderHeight);
    microTuneLabel.setBounds(microTuneSlider.getX(), microTuneSlider.getY() - 15, sliderWidth, 20);

    volumeSlider.setBounds(cleaningNoiseSlider.getRight(), SliderSide - 55, sliderWidth, sliderHeight);
    volumeLabel.setBounds(cleaningNoiseSlider.getRight(), SliderSide - 65, sliderWidth, 20);
